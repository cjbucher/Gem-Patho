#!/usr/bin/env python
"""
Transformer-based survival model with image modality (ABMIL-inspired) added.
...
Model outputs (for multiple hyperparameter experiments) will be saved in:
 /n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/actual_final_results/only_image/ct_batch
Note: The input data is now assumed to include "Project ID" (for image data)
      along with "Case ID", "gene_embed_seq", "OS.time", "OS", "type", and "description_embeddings".
"""
...
BASE_OUTPUT_DIR = "/n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/actual_final_results/only_image/ct_batch"
NUM_FOLDS = 10
...
##########################################
# Dataset for Preprocessed Sequences with Image Data
##########################################
class PreprocessedSequenceDataset(Dataset):
    def __init__(self, df, token_col="gene_embed_seq", cancer_type_mapping=None):
        ...
        Expects a DataFrame with columns:
          ...
          - "Project ID": project identifier (for image file lookup).
    ...
    def __getitem__(self, idx):
        ...
        # Load image features from FS file using "Project ID" and "Case ID".
        ...
        if fs_files:
            ...
            # Fallback to a zero tensor if no image file is found.
            img_features = torch.zeros((1, 1536), dtype=torch.float)
        
        # Return image features in place of case_id.
        return embeddings, scores, cnas, cancer_type_tensor, description, time, event, img_features

##########################################
# Collate Function for Padding (with Image Data)
##########################################
...
class PreprocessedTransformerSurvivalModel(nn.Module):
    ...
    def forward(self, emb, scores, cnas, cancer_type, description, image_features, src_key_padding_mask=None):
        ...
        # Adjust padding mask: prepend mask for description token.
        ...
        risk = self.fusion_linear(img_feats)  # (B, 1)
        return risk
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, img_list in dataloader:
            ...
            # We no longer have case_ids here.
    ...
    return avg_loss, global_cindex, cindex_by_type, normalized_avg_cindex, all_T, all_risk, all_ct, missing_cancer_types
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20,
    ...
    for epoch in range(1, max_epochs+1):
        ...
        train_losses = []
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, img_list in current_loader:
            emb_batch = emb_batch.to(device)
            score_batch = score_batch.to(device)
            cna_batch = cna_batch.to(device)
            cancer_type_batch = cancer_type_batch.to(device)
        ...
        val_loss, val_cindex_global, val_cindex_by_type, normalized_avg_cindex, _, _, _, missing_ct = evaluate_model(
...
def main():
    ...
    # Now require "Project ID" for image modality.
    ...
    for split, loader, prefix in zip(["train", "val"], [train_loader, val_loader], ["train", "val"]):
        avg_loss, global_cindex, cindex_by_type, normalized_avg_cindex, all_T, all_risk, all_ct, missing_ct = evaluate_model(model, loader, device)
        df_out = pd.DataFrame({
            "OS.time": all_T,
            "risk": all_risk,
    ...
    train_loss, train_cindex, _, _, _, _, _, _ = evaluate_model(model, train_loader, device)
    val_loss, val_cindex_global, _, normalized_avg_cindex, _, _, _, _ = evaluate_model(model, val_loader, device)
    test_loss, test_cindex, test_T, test_risk, test_ct, _, _, _ = evaluate_model(model, test_loader, device)
