...
Model outputs (for multiple hyperparameter experiments) will be saved in:
  /n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/actual_final_results/only_genetics/all_batch
"""
...
BASE_OUTPUT_DIR = "/n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/actual_final_results/only_genetics/all_batch"
NUM_FOLDS = 10
...
def train_model_fn(train_loader, val_loader, model, device,
    ...
    # ----------------------------
    # Phase 2: Cancer type specific batching
    # ----------------------------
    ...
    if phase2_epochs > 0:
        ...
        for epoch in range(1, phase2_epochs+1):
            model.train()
            phase2_losses = []
            for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, _ in phase2_loader:
                emb_batch = emb_batch.to(device)
                score_batch = score_batch.to(device)
                cna_batch = cna_batch.to(device)
                cancer_type_batch = cancer_type_batch.to(device)
...
def main():
    ...
    train_df = pd.read_parquet(os.path.join(fold_folder, train_path), engine="pyarrow")[cols]
    val_df = pd.read_parquet(os.path.join(fold_folder, val_path), engine="pyarrow")[cols]
    test_df = pd.read_parquet(os.path.join(fold_folder, test_path), engine="pyarrow")[cols]
