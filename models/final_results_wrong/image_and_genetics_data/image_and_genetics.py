...
Model outputs (for multiple hyperparameter experiments) will be saved in:
  /home/chb3333/yulab/chb3333/gem-patho/models/final_results/image_and_genetics_data
"""
...
BASE_OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/final_results/image_and_genetics_data"
NUM_FOLDS = 10
...
class PreprocessedSequenceDataset(Dataset):
    def __init__(self, df, token_col="gene_embed_seq", cancer_type_mapping=None):
        ...
        Expects a DataFrame with columns:
          ...
          - token_col: a list/array of tokens.
    ...
    def __getitem__(self, idx):
        ...
        return embeddings, scores, cnas, cancer_type_tensor, description, time, event, row.get("Case ID", "Unknown"), img_features
...
def collate_fn_preprocessed(batch):
    ...
    return padded_emb, padded_scores, padded_cnas, cancer_types, descriptions, times, events, mask, list(case_ids), list(img_list)
...
class PreprocessedTransformerSurvivalModel(nn.Module):
    def __init__(self, d_gene, d_model=256, polyphen_hidden_dim=128, nhead=4, dropout=0.1, desc_dim=None):
        ...
        # --- Image extractor and fusion layer ---
    ...
    def forward(self, emb, scores, cnas, cancer_type, description, src_key_padding_mask=None, image_features=None):
        ...
        pooled = transformer_out[:, 0, :]  # pooled token representation
        
        # Process image features.
        img_feats = []
        for img in image_features:  # each img is a tensor of shape (N, 1536)
            img_feat = self.image_extractor(img)  # output shape: (d_model,)
...
def evaluate_model(model, dataloader, device, final_percentage=0.8):
    ...
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, case_ids, img_list in dataloader:
            ...
            risk = model(emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch,
                         src_key_padding_mask=mask, image_features=img_list)
    ...
    return avg_loss, global_cindex, all_T, all_risk, all_ct, all_case_ids
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20,
    ...
    for epoch in range(1, max_epochs+1):
        ...
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, _ , img_list in train_loader:
...
def main():
    ...
    # Include "Case ID" and "Project ID" columns.
    cols = ["Case ID", "Project ID", "gene_embed_seq", "OS.time", "OS", "type", "description_embeddings"]
    ...
    sample_emb, sample_scores, sample_cnas, sample_cancer_type, sample_desc, sample_time, sample_event, sample_mask, _, sample_img = next(iter(train_loader))
