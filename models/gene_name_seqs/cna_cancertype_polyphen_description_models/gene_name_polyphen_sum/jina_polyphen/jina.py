...
OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/gene_name_seqs/cna_cancertype_polyphen_description_models/gene_name_polyphen_sum/jina_polyphen"
NUM_FOLDS = 10
...
class PreprocessedSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.has_description:
            # Assuming description_embeddings is already a vector.
            description = torch.tensor(row["description_embeddings"], dtype=torch.float)
        else:
            # If not present, use a zero vector; note: you may adjust the dimension accordingly.
            print("NNNNNNNNOOOOOOOOOO Description Embedding")
...
# with polyphen, CNA, and cancer type MLPs, plus a separate description token.
##########################################
class PreprocessedTransformerSurvivalModel(nn.Module):
    ...
    def forward(self, emb, scores, cnas, cancer_type, description, src_key_padding_mask=None):
        ...
        # Sum the gene token projections.
...
def main():
    ...
    for fold in range(1, NUM_FOLDS+1):
        ...
        # Load train, val, test parquet files from the fold folder.
