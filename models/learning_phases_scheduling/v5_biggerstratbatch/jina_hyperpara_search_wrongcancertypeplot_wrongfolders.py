...
# Constants & Paths
##########################################
# Added "Case ID" to input columns later
...
def evaluate_model(model, dataloader, device, final_percentage=0.8):
    ...
    return avg_loss, global_cindex, cindex_by_type, normalized_avg_cindex, all_T, all_risk, all_ct, all_case_ids
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20,
    ...
    for epoch in range(1, max_epochs+1):
        ...
        val_loss, val_cindex_global, val_cindex_by_type, normalized_avg_cindex, _, _, _, _ = evaluate_model(model, val_loader, device, final_percentage=final_percentage)
        history.append({
            "epoch": epoch,
...
def main():
    ...
    for split, loader, prefix in zip(["train", "val"], [train_loader, val_loader], ["train", "val"]):
        avg_loss, global_cindex, cindex_by_type, normalized_avg_cindex, all_T, all_risk, all_ct, all_case_ids = evaluate_model(model, loader, device, final_percentage=exp["final_percentage"])
        df_out = pd.DataFrame({
            "Case ID": all_case_ids,
            "OS.time": all_T,
            "risk": all_risk,
            "cancer_type": all_ct,  # integer label from argmax
            "split": prefix
        })
    ...
    train_loss, train_cindex, _, _, _, _, _, _ = evaluate_model(model, train_loader, device, final_percentage=exp["final_percentage"])
    val_loss, val_cindex_global, _, normalized_avg_cindex, _, _, _, _ = evaluate_model(model, val_loader, device, final_percentage=exp["final_percentage"])
    test_loss, test_cindex, test_T, test_risk, _, _, _, _ = evaluate_model(model, test_loader, device, final_percentage=exp["final_percentage"])
