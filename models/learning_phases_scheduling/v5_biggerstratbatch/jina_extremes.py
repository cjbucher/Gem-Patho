...
class PhaseAwareLoss:
    ...
    def compute(risk, times, events, cancer_type_indices, phase=0,
        ...
        # For phases 1 and 2, scale loss per sample by the inverse of the count for that cancer type.
        if phase > 0:
...
def main():
    ...
    exp_dir = os.path.join(BASE_OUTPUT_DIR, f"exp_{experiment_idx}_tp_{exp['transition_percentage']}_fp_{exp['final_percentage']}_warmup_{exp['warmup']}_transition_{exp['transition']}")
    os.makedirs(exp_dir, exist_ok=True)
    fold_dir = os.path.join(BASE_OUTPUT_DIR, f"exp_{experiment_idx}_fold_{fold}")
