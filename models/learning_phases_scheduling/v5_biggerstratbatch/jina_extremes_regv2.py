...
Model outputs (for multiple hyperparameter experiments) will be saved in:
  /home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v5_biggerstratbatch
"""
...
BASE_OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v5_biggerstratbatch"
NUM_FOLDS = 10
...
class PhaseAwareLoss:
    ...
    def compute(risk, times, events, cancer_type_indices, model, phase=0,
        ...
        Args:
            risk (Tensor): Predicted risk values, shape (B,).
            times (Tensor): Observed times, shape (B,).
            events (Tensor): Event indicators (0/1), shape (B,).
            cancer_type_indices (Tensor): Tensor with indices indicating cancer types (or -1 for invalid entries).
            model (torch.nn.Module): The model whose parameters will be regularized.
            phase (int): Phase indicator (0 for baseline, 1 or 2 for later phases).
            transition_percentage (float): Used in phase 1 to randomly downweight pairs.
            final_percentage (float): Used in phase 2 to randomly downweight pairs.
            lambda_reg (float): Regularization coefficient.
        
        Returns:
            loss (Tensor): The computed loss including the regularization term.
        """
        B = risk.shape[0]
        risk = torch.clamp(risk, min=-50, max=50)
        
        # Compute pairwise differences of times.
        diff = times.unsqueeze(0) - times.unsqueeze(1)
        mat_A = (diff > 0).float()
        
        if phase == 0:
            # In phase 0, use equality matrix for ties and disable upper triangle comparisons.
            mat_B = (diff == 0).float()
            for i in range(B):
                mat_B[i, i+1:] = 0
            pair_mask = torch.ones((B, B), device=risk.device)
        else:
            # For phase 1 and 2, only use upper triangle for ties.
        ...
        # Apply pair mask to both matrices.
        mat_A *= pair_mask
        mat_B *= pair_mask
        
        exp_risk = torch.exp(risk)
        # Compute the denominator for the loss.
        ...
        if phase > 0:
            ...
            scale = 1.0 / counts[inv].float()  # Each sample gets a weight = 1/count
            loss = -torch.mean(scale * events * (risk.squeeze() - torch.log(R)))
        else:
            loss = -torch.mean(events * (risk.squeeze() - torch.log(R)))
        
        # Compute the L2 regularization loss over model parameters.
        reg_loss = 0.0
...
def collate_fn_preprocessed(batch):
    ...
    return padded_emb, padded_scores, padded_cnas, cancer_types, descriptions, times, events, mask, list(case_ids)
...
def evaluate_model(model, dataloader, device, final_percentage=0.8):
    ...
    losses = []
    missing_cancer_types = []  # To record cancer types for which c-index couldn't be computed
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, case_ids in dataloader:
            ...
            # Here, we want to derive an integer label from our binary cancer_type representation.
            # Instead of using argmax (which only gives values 0-5), we can convert the binary vector to an integer.
            # For simplicity, we use the mapping we defined: type_to_index.
            # Since our dataset already converts cancer type to a binary tensor based on cancer_type_mapping,
            # we can try to reverse it by comparing with our mapping.
            ct_idx_list = []
            for ct_tensor in cancer_type_batch:
                ct_list = ct_tensor.cpu().tolist()
                # Find the matching key in cancer_type_mapping (if any)
                found = False
            ...
            loss = PhaseAwareLoss.compute(risk, T_batch, E_batch, ct_idx, model, phase=2, final_percentage=final_percentage)
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20,
    ...
    for epoch in range(1, max_epochs+1):
        ...
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask, _ in train_loader:
            emb_batch = emb_batch.to(device)
            score_batch = score_batch.to(device)
            cna_batch = cna_batch.to(device)
            cancer_type_batch = cancer_type_batch.to(device)
            ...
            # Use mapping to convert binary vector to index
            ct_idx_list = []
            for ct_tensor in cancer_type_batch:
                ct_list = ct_tensor.cpu().tolist()
                found = False
            ...
            loss = PhaseAwareLoss.compute(risk, T_batch, E_batch, ct_idx, model, phase=phase,
        ...
        val_loss, val_cindex_global, val_cindex_by_type, normalized_avg_cindex, _, _, _, _, missing_ct = evaluate_model(
            model, val_loader, device, final_percentage=final_percentage)
        history.append({
            "epoch": epoch,
...
def main():
    ...
    if not torch.cuda.is_available():
        ...
        # Optionally exit or continue on CPU.
        # import sys
        # sys.exit(1)

    experiments = [
        {"transition_percentage": 0.8, "final_percentage": 0.85, "warmup": 10, "transition": 10},
        {"transition_percentage": 0.8, "final_percentage": 0.75, "warmup": 10, "transition": 10},
    ]
    ...
    sample_emb, sample_scores, sample_cnas, sample_cancer_type, sample_desc, sample_time, sample_event, sample_mask, _ = next(iter(train_loader))
    ...
    exp_dir = os.path.join(BASE_OUTPUT_DIR, f"regularizationloss_exp_{experiment_idx}_tp_{exp['transition_percentage']}_fp_{exp['final_percentage']}_warmup_{exp['warmup']}_transition_{exp['transition']}")
    os.makedirs(exp_dir, exist_ok=True)
    # Create fold subfolder inside the experiment folder.
    ...
    for split, loader, prefix in zip(["train", "val"], [train_loader, val_loader], ["train", "val"]):
        avg_loss, global_cindex, cindex_by_type, normalized_avg_cindex, all_T, all_risk, all_ct, all_case_ids, missing_ct = evaluate_model(model, loader, device, final_percentage=exp["final_percentage"])
        df_out = pd.DataFrame({
        ...
        # Also save missing cancer types for this split.
    ...
    train_loss, train_cindex, _, _, _, _, _, _, _ = evaluate_model(model, train_loader, device, final_percentage=exp["final_percentage"])
    val_loss, val_cindex_global, _, normalized_avg_cindex, _, _, _, _, _ = evaluate_model(model, val_loader, device, final_percentage=exp["final_percentage"])
    test_loss, test_cindex, test_T, test_risk, _, _, _, _, _ = evaluate_model(model, test_loader, device, final_percentage=exp["final_percentage"])
