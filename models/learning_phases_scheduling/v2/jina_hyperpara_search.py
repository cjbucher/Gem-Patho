...
Model outputs (for multiple hyperparameter experiments) will be saved in:
  /home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v2
"""
...
BASE_OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v2"
NUM_FOLDS = 10
    ...
    def compute(risk, times, events, cancer_type_indices, phase=0, same_type_weight=2.0):
        ...
        if phase == 0:
            ...
            if phase == 1:
                ...
                # In phase 1, include different-type pairs only 25% of the time.
                rand_mask = (torch.rand_like(same_type) < 0.75).float()
                # For same-type pairs: weight = 1; for different-type: weight = 1 - rand_mask.
                pair_mask = (same_type * 1 + (1 - same_type) * (1 - rand_mask)) * valid_mask
            else:  # phase 2: include all pairs, but weight same-type pairs higher.
                same_type = (cancer_type_indices.unsqueeze(1) == cancer_type_indices.unsqueeze(0)).float()
...
##########################################
# Evaluation Function (accepting hyperparameter same_type_weight)
##########################################
def evaluate_model(model, dataloader, device, same_type_weight=2.0):
    model.eval()
    ...
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask in dataloader:
            ...
            loss = PhaseAwareLoss.compute(risk, T_batch, E_batch, ct_idx, phase=2, same_type_weight=same_type_weight)
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20,
                   same_type_weight=2.0, warmup=10, transition=10):
    ...
    for epoch in range(1, max_epochs+1):
        ...
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask in train_loader:
            ...
            optimizer.zero_grad()
            risk = model(emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, src_key_padding_mask=mask)
            loss = PhaseAwareLoss.compute(risk, T_batch, E_batch, ct_idx, phase=phase, same_type_weight=same_type_weight)
            loss.backward()
        ...
        val_loss, val_cindex, _, _ = evaluate_model(model, val_loader, device, same_type_weight=same_type_weight)
...
def main():
    ...
    experiments = [
        {"same_type_weight": 2.0, "warmup": 10, "transition": 10},
        {"same_type_weight": 2.5, "warmup": 10, "transition": 10},
        {"same_type_weight": 2.0, "warmup": 5, "transition": 10},
        {"same_type_weight": 1.5, "warmup": 5, "transition": 10},
        {"same_type_weight": 1.5, "warmup": 5, "transition": 15},
        {"same_type_weight": 2.0, "warmup": 15, "transition": 10},
        {"same_type_weight": 1.5, "warmup": 15, "transition": 10},
        {"same_type_weight": 2.0, "warmup": 10, "transition": 15},

        {"same_type_weight": 1.0, "warmup": 10, "transition": 10},  # Baseline: no extra weighting in phase 2.
        {"same_type_weight": 2.0, "warmup": 0, "transition": 0},    # Immediate phase 2 from the start.
        {"same_type_weight": 2.0, "warmup": 3, "transition": 5},    # Very short warmup and transition.
        {"same_type_weight": 3.0, "warmup": 10, "transition": 10},  # Higher same-type weight.
        {"same_type_weight": 2.0, "warmup": 20, "transition": 10},  # Longer warmup.
        {"same_type_weight": 2.0, "warmup": 10, "transition": 20},  # Longer transition.
    ]
    
    # Create a summary list for all experiments.
    all_experiment_results = []
    
    # Loop over experiments.
    for exp_idx, exp in enumerate(experiments):
        exp_dir = os.path.join(BASE_OUTPUT_DIR, f"exp_{exp_idx}_weight_{exp['same_type_weight']}_warmup_{exp['warmup']}_transition_{exp['transition']}")
        os.makedirs(exp_dir, exist_ok=True)
        print(f"\n=== Running Experiment {exp_idx}: {exp} ===")
        exp_fold_metrics = []
        ...
        for fold in range(1, NUM_FOLDS+1):
            ...
            model, best_epoch, best_val_loss, history = train_model_fn(train_loader, val_loader, model, device,
                                                                       ...
                                                                       same_type_weight=exp["same_type_weight"],
                                                                       warmup=exp["warmup"],
            ...
            train_loss, train_cindex, _, _ = evaluate_model(model, train_loader, device, same_type_weight=exp["same_type_weight"])
            val_loss, val_cindex, _, _ = evaluate_model(model, val_loader, device, same_type_weight=exp["same_type_weight"])
            test_loss, test_cindex, test_T, test_risk = evaluate_model(model, test_loader, device, same_type_weight=exp["same_type_weight"])
