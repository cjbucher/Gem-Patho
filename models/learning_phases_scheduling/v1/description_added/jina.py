...
Model outputs will be saved in:
  /home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v1/description_added
...
OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v1/description_added"
print(OUTPUT_DIR)
NUM_FOLDS = 10
...
class PhaseAwareLoss:
    ...
    def compute(risk, times, events, cancer_type_indices, phase=0, same_type_weight=2.0):
        ...
        if phase == 0:
            mat_B = (diff == 0).float()
            # Zero out upper triangle of mat_B.
            for i in range(B):
                mat_B[i, i+1:] = 0
            # No masking from cancer types.
            pair_mask = torch.ones((B, B), device=risk.device)
        else:
            # For phases 1 and 2, only consider lower triangle for equal times.
            ...
            if phase == 1:
                ...
                # Use a random mask where different-type pairs are included with 25% chance.
                rand_mask = (torch.rand_like(same_type) < 0.75).float()
                # For same-type pairs, we always want a weight of 1.
                # For different-type pairs, the mask is 1 - rand_mask, so expectation is 0.25.
                pair_mask = (same_type * rand_mask + (1 - rand_mask)) * valid_mask
            else:  # phase 2
                same_type = (cancer_type_indices.unsqueeze(1) == cancer_type_indices.unsqueeze(0)).float()
                # All pairs are included. Same-type pairs are weighted higher.
                # For same-type pairs: weight = 1 + (same_type_weight - 1) = same_type_weight.
                # For different-type pairs: weight = 1.
                pair_mask = (1 + same_type * (same_type_weight - 1)) * valid_mask
        
        # Apply the mask to the pair comparisons.
        mat_A *= pair_mask
        mat_B *= pair_mask
        
        exp_risk = torch.exp(risk)
        # Compute R as the sum over pairs.
...
##########################################
# Transformer Survival Model for Preprocessed Input (with Description Token)
...
class PreprocessedTransformerSurvivalModel(nn.Module):
    ...
    def forward(self, emb, scores, cnas, cancer_type, description, src_key_padding_mask=None):
        ...
        # Sum projections for gene tokens.
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask in dataloader:
            ...
            # Convert cancer type vector to indices.
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20):
    ...
    for epoch in range(1, max_epochs+1):
        ...
        for emb_batch, score_batch, cna_batch, cancer_type_batch, desc_batch, T_batch, E_batch, mask in train_loader:
            ...
            # Convert cancer type to indices.
            ct_idx = torch.argmax(cancer_type_batch, dim=1)
...
def main():
    ...
    for fold in range(1, NUM_FOLDS+1):
        ...
        # Keep required columns (including description embeddings).
