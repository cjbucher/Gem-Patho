...
Data files are now assumed to be in:
  ...
  Model outputs will be saved in:
    /home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v1/no_description
...
OUTPUT_DIR = "/home/chb3333/yulab/chb3333/gem-patho/models/learning_phases_scheduling/v1/no_description"
print(OUTPUT_DIR)
NUM_FOLDS = 10
...
class PhaseAwareLoss:
    ...
    def compute(risk, times, events, cancer_type_indices, phase=0, same_type_weight=2.0):
        ...
        # For phase 0, mimic partial_ranking_loss exactly:
        if phase == 0:
            # Compute diff so that diff[i,j] = times[j] - times[i]
            diff = times.unsqueeze(0) - times.unsqueeze(1)
            # mat_A: pairs where the later time is greater than the earlier time
            mat_A = (diff > 0).float()
            # mat_B: pairs with equal times
            mat_B = (diff == 0).float()
            # Zero out the upper triangle of mat_B (for each row, set columns i+1 to end to 0)
            for i in range(B):
                mat_B[i, i+1:] = 0
            # No masking from cancer types in phase 0:
            pair_mask = torch.ones((B, B), device=risk.device)
        else:
            # (Existing phase 1 and 2 code remains unchanged.)
            diff = times.unsqueeze(0) - times.unsqueeze(1)
            mat_A = (diff > 0).float()
            ...
            if phase == 1:
                ...
                rand_mask = (torch.rand_like(same_type) < 0.5).float()
                pair_mask = (same_type * rand_mask + (1 - rand_mask)) * valid_mask
            else:  # phase 2
                same_type = (cancer_type_indices.unsqueeze(1) == cancer_type_indices.unsqueeze(0)).float()
                pair_mask = same_type * valid_mask
                mat_A *= same_type_weight
                mat_B *= same_type_weight
        
        # Apply the mask (for phase 0 this is just ones)
        mat_A *= pair_mask
        mat_B *= pair_mask
        
        exp_risk = torch.exp(risk)
        # Sum over the pairs as in the original loss
...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for emb_batch, score_batch, cna_batch, cancer_type_batch, T_batch, E_batch, mask in dataloader:
            ...
            ct_idx = torch.argmax(cancer_type_batch, dim=1)  # Convert 6D vector to indices
...
class PhaseScheduler:
    def __init__(self, warmup=10, transition=5):
        self.warmup = warmup
        self.transition = transition
        
    def get_phase(self, epoch):
        if epoch < self.warmup: return 0
        if epoch < self.warmup + self.transition: return 1
        return 2
...
def train_model_fn(train_loader, val_loader, model, device, max_epochs=100, patience=20):
    ...
    scheduler = PhaseScheduler(warmup=10, transition=5)
    ...
    best_epoch = 0
    epochs_no_improve = 0
    best_model_state = copy.deepcopy(model.state_dict())  # Save initial model state    


    # Keep original training loop structure
    for epoch in range(1, max_epochs+1):
        ...
        for emb_batch, score_batch, cna_batch, cancer_type_batch, T_batch, E_batch, mask in train_loader:
            # Original device movement
            inputs = [x.to(device) for x in (emb_batch, score_batch, cna_batch, cancer_type_batch, T_batch, E_batch, mask)]
            emb, scores, cnas, ct_vec, T, E, mask = inputs
            
            # Get cancer type indices (added)
            ct_idx = torch.argmax(ct_vec, dim=1)  # Convert one-hot to indices
            
            optimizer.zero_grad()
            risk = model(emb, scores, cnas, ct_vec, mask)
            
            # Modified loss call
            loss = PhaseAwareLoss.compute(risk, T, E, ct_idx, phase=phase)
            
            # Keep original backward logic
            loss.backward()
