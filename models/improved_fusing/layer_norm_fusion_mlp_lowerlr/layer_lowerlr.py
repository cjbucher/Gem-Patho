...
Model outputs (for multiple hyperparameter experiments) will be saved in:
 /n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/improved_fusing/layer_norm_fusion_mlp
"""
...
BASE_OUTPUT_DIR = "/n/data2/hms/dbmi/kyu/lab/chb3333/gem-patho/models/improved_fusing/layer_norm_fusion_mlp_lowerlr"
NUM_FOLDS = 10
...
##########################################
def compute_loss(risk, times, events, cancer_type_indices, model, lambda_reg=1e-5):
...
class PreprocessedTransformerSurvivalModel(nn.Module):
    def __init__(self, d_gene, d_model=256, polyphen_hidden_dim=128, nhead=4, dropout=0.1, desc_dim=None):
        ...
        # --- Modality projections with LayerNorm ---
        self.gene_linear = nn.Sequential(
            nn.Linear(d_gene, d_model),
            nn.LayerNorm(d_model) # norm
        )
        ...
        if desc_dim is None:
            desc_dim = d_gene
        self.description_linear = nn.Sequential(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=nhead, dropout=dropout, activation="gelu"
        )
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=2)
        self.transformer_pool_norm = nn.LayerNorm(d_model)  # norm
        
        # --- Image Modality ---
        self.image_extractor = ImageFeatureExtractor(D_feat=1536, D_inner=512, D_out=d_model, droprate=0.1)
        self.image_norm = nn.LayerNorm(d_model)  # norm
        
        # --- Fusion ---
        self.fusion_norm = nn.LayerNorm(2 * d_model)  # norm
        self.fusion_mlp = nn.Sequential(
            nn.Linear(2 * d_model, d_model),  # first layer
            nn.GELU(),
            nn.Linear(d_model, d_model),       # added extra hidden layer
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(d_model, 1)              # output layer
        )
        # Apply Xavier uniform initialization with small gain to the first and final layers.
        nn.init.xavier_uniform_(self.fusion_mlp[0].weight, gain=0.01)
        nn.init.xavier_uniform_(self.fusion_mlp[-1].weight, gain=0.01)
    ...
    def forward(self, emb, scores, cnas, cancer_type, description, image_features):
        ...
        # Run transformer encoder (note: you can consider setting batch_first=True in the encoder layer if desired)
        token_emb = token_emb.transpose(0, 1)
        transformer_out = self.transformer_encoder(token_emb)
        transformer_out = transformer_out.transpose(0, 1)
        pooled = self.transformer_pool_norm(transformer_out[:, 0, :])

        # Process image features one by one in a loop (since each image has a variable number of tiles)
        img_feats = []
        for img in image_features:
            # Each `img` is a tensor of shape (n_tiles, 1536). The image extractor will aggregate these tiles.
            img_feat = self.image_extractor(img)
            img_feats.append(img_feat)
        img_feats = torch.stack(img_feats, dim=0)
        img_feats = self.image_norm(img_feats)  # apply normalization to aggregated image features

        # Fuse modalities
        fusion = torch.cat([pooled, img_feats], dim=1)
        fusion = self.fusion_norm(fusion)
        risk = self.fusion_mlp(fusion)
        return risk
...
def train_model_fn(train_loader, val_loader, model, device,
    ...
    for epoch in range(1, max_phase0_epochs+1):
        ...
        if val_loss < best_val_loss or normalized_avg_cindex > best_cindex:
            best_val_loss = min(best_val_loss, val_loss)
            best_cindex = max(best_cindex, normalized_avg_cindex)
            best_epoch = epoch
            best_model_state = copy.deepcopy(model.state_dict())
            epochs_no_improve = 0
        else:
